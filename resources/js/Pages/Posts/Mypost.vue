...
const postsFiltrados = computed(() => {
  ...
  if (ordenActivo.value === 'antiguas') {
    posts = posts.sort((a, b) => new Date(a.created_at) - new Date(b.created_at)); // Orden ascendente
  } else {
    posts = posts.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)); // Orden descendente
  }
  ...
  <AppLayout title="Mis Posts">
    ...
    <div class="min-h-screen flex flex-col">
      <div class="container mx-auto mt-4 px-4 flex-grow">
        ...
        <!-- Listado de posts -->
        <div v-if="postsFiltrados.some(cat => cat.posts.length > 0)" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-6">
            ...
            >
              <div>
                ...
                <div v-if="post.imagen">
                  <img :src="`/storage/${post.imagen}`" alt="Imagen del post" class="w-full aspect-video object-cover rounded" />
                </div>
              </div>
              <div class="mt-4 sm:mt-0 text-right">
                <Link :href="route('posts.show', post.id)" class="text-sm text-blue-600 font-medium hover:underline">Leer más →</Link>
              </div>
            </div>
          </template>
        </div>
